...
export default function PagePartManager({ pageType }: PagePartManagerProps) {
  ...
  const templates = isHeader ? headerTemplates : footerTemplates;
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const reason =
          response.status === 401
            ...
            : response.status >= 500
              ? 'Server error — check the dev console for the API stack trace.'
              : body || `HTTP ${response.status}`;
        throw new Error(`get-pages failed (${response.status}): ${reason}`);
      }
      ...
      toast.error(error instanceof Error ? error.message : `Failed to load ${pageType}s`);
    } finally {
      setIsLoading(false);
    }
  }, [pageType]);
      ...
      <div className="grid gap-6 lg:grid-cols-[520px_1fr]">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>{isHeader ? 'Header Style' : 'Footer Style'}</Label>
