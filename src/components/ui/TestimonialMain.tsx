'use client';

import React from 'react';
import { useAppDispatch } from '@/redux/hooks';
import { updateBlockContent } from '@/redux/canvasSlice';
import { InlineEditableText } from '@/components/editor/InlineEditableText';
<<<<<<< HEAD
import { EditableElement } from '@/components/editor/EditableElement';
=======
>>>>>>> khadija
import type { BlockRendererProps } from '@/types/index';
import type { BlockContentItem } from '@/types/blockContent';

export const TestimonialMain = ({ block, isEditing = false }: BlockRendererProps) => {
  const dispatch = useAppDispatch();
  
  const content = React.useMemo(() => {
    try {
      return typeof block.content === 'string' && block.content.startsWith('{')
        ? JSON.parse(block.content)
        : {};
    } catch {
      return {};
    }
  }, [block.content]);

  const testimonials = content.testimonials || [
    { 
      text: "Synth chartreuse iPhone lomo cray raw denim brunch everyday carry neutra before they sold out fixie 90's microdosing. Tacos pinterest fanny pack venmo, post-ironic heirloom try-hard pabst authentic iceland.", 
      name: "Holden Caulfield", 
      role: "UI DEVELOPER",
      image: "https://dummyimage.com/106x106"
    },
    { 
      text: "Synth chartreuse iPhone lomo cray raw denim brunch everyday carry neutra before they sold out fixie 90's microdosing. Tacos pinterest fanny pack venmo, post-ironic heirloom try-hard pabst authentic iceland.", 
      name: "Alper Kamu", 
      role: "DESIGNER",
      image: "https://dummyimage.com/107x107"
    },
  ];

  const handleUpdate = (key: string, newValue: unknown) => {
    if (!isEditing) return;
    const updatedContent = { ...content, [key]: newValue };
    dispatch(
      updateBlockContent({
        id: block.uniqueId ?? '',
        content: JSON.stringify(updatedContent),
      })
    );
  };

  const updateTestimonial = (index: number, key: string, value: string) => {
    if (!isEditing) return;
    const newTestimonials = [...testimonials];
    newTestimonials[index] = { ...newTestimonials[index], [key]: value };
    handleUpdate('testimonials', newTestimonials);
  };

  return (
<<<<<<< HEAD
    <section className="text-[color:var(--color-muted,#4b5563)] body-font w-full" style={block.style}>
=======
    <section className="text-gray-600 body-font w-full" style={block.style}>
>>>>>>> khadija
      <div className="container px-5 py-24 mx-auto">
        <InlineEditableText
          tagName="h1"
          value={content.mainTitle || 'Testimonials'}
          onBlur={(v) => handleUpdate('mainTitle', v)}
          isEditing={isEditing}
<<<<<<< HEAD
          className="text-3xl font-medium title-font text-[color:var(--color-text,#111827)] mb-12 text-center"
          style={{ 
            color: block.style?.color || 'var(--color-text, #111827)', 
=======
          className="text-3xl font-medium title-font text-gray-900 mb-12 text-center"
          style={{ 
            color: block.style?.color, 
>>>>>>> khadija
            fontFamily: block.style?.fontFamily,
            ...content.mainTitleStyle 
          }}
        />
        <div className="flex flex-wrap -m-4">
          {testimonials.map((testimonial: BlockContentItem, index: number) => (
            <div key={index} className="p-4 md:w-1/2 w-full">
<<<<<<< HEAD
              <EditableElement
                block={block}
                isEditing={isEditing}
                path={`testimonials.${index}.cardStyle`}
                className="h-full bg-[var(--color-surface,#f3f4f6)] p-8 rounded"
              >
                <EditableElement
                  block={block}
                  isEditing={isEditing}
                  path={`testimonials.${index}.iconStyle`}
                  className="block w-5 h-5 text-[color:var(--color-muted,#9ca3af)] mb-4"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-full h-full" viewBox="0 0 975.036 975.036">
                    <path d="M925.036 57.197h-304c-27.6 0-50 22.4-50 50v304c0 27.601 22.4 50 50 50h145.5c-1.9 79.601-20.4 143.3-55.4 191.2-27.6 37.8-69.399 69.1-125.3 93.8-25.7 11.3-36.8 41.7-24.8 67.101l36 76c11.6 24.399 40.3 35.1 65.1 24.399 66.2-28.6 122.101-64.8 167.7-108.8 55.601-53.7 93.7-114.3 114.3-181.9 20.601-67.6 30.9-159.8 30.9-276.8v-239c0-27.599-22.401-50-50-50zM106.036 913.497c65.4-28.5 121-64.699 166.9-108.6 56.1-53.7 94.4-114.1 115-181.2 20.6-67.1 30.899-159.6 30.899-277.5v-239c0-27.6-22.399-50-50-50h-304c-27.6 0-50 22.4-50 50v304c0 27.601 22.4 50 50 50h145.5c-1.9 79.601-20.4 143.3-55.4 191.2-27.6 37.8-69.4 69.1-125.3 93.8-25.7 11.3-36.8 41.7-24.8 67.101l35.9 75.8c11.601 24.399 40.501 35.2 65.301 24.399z"></path>
                  </svg>
                </EditableElement>
=======
              <div className="h-full bg-gray-100 p-8 rounded">
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="block w-5 h-5 text-gray-400 mb-4" viewBox="0 0 975.036 975.036">
                  <path d="M925.036 57.197h-304c-27.6 0-50 22.4-50 50v304c0 27.601 22.4 50 50 50h145.5c-1.9 79.601-20.4 143.3-55.4 191.2-27.6 37.8-69.399 69.1-125.3 93.8-25.7 11.3-36.8 41.7-24.8 67.101l36 76c11.6 24.399 40.3 35.1 65.1 24.399 66.2-28.6 122.101-64.8 167.7-108.8 55.601-53.7 93.7-114.3 114.3-181.9 20.601-67.6 30.9-159.8 30.9-276.8v-239c0-27.599-22.401-50-50-50zM106.036 913.497c65.4-28.5 121-64.699 166.9-108.6 56.1-53.7 94.4-114.1 115-181.2 20.6-67.1 30.899-159.6 30.899-277.5v-239c0-27.6-22.399-50-50-50h-304c-27.6 0-50 22.4-50 50v304c0 27.601 22.4 50 50 50h145.5c-1.9 79.601-20.4 143.3-55.4 191.2-27.6 37.8-69.4 69.1-125.3 93.8-25.7 11.3-36.8 41.7-24.8 67.101l35.9 75.8c11.601 24.399 40.501 35.2 65.301 24.399z"></path>
                </svg>
>>>>>>> khadija
                <InlineEditableText
                  tagName="p"
                  value={testimonial.text}
                  onBlur={(v) => updateTestimonial(index, 'text', v)}
                  isEditing={isEditing}
                  className="leading-relaxed mb-6"
<<<<<<< HEAD
                  style={{
                    color: block.style?.color || 'var(--color-text, #111827)',
                    fontFamily: block.style?.fontFamily,
                    ...content.quoteStyle
                  }}
                />
                <div className="inline-flex items-center">
                  <EditableElement
                    block={block}
                    isEditing={isEditing}
                    path={`testimonials.${index}.imageStyle`}
                    className="w-12 h-12 rounded-full flex-shrink-0 overflow-hidden"
                  >
                    <img alt="testimonial" src={testimonial.image || "https://dummyimage.com/106x106"} className="w-full h-full object-cover object-center" />
                  </EditableElement>
=======
                  style={{ 
                    color: block.style?.color, 
                    fontFamily: block.style?.fontFamily,
                    ...content.quoteStyle 
                  }}
                />
                <div className="inline-flex items-center">
                  <img alt="testimonial" src={testimonial.image || "https://dummyimage.com/106x106"} className="w-12 h-12 rounded-full flex-shrink-0 object-cover object-center" />
>>>>>>> khadija
                  <span className="flex-grow flex flex-col pl-4">
                    <InlineEditableText
                      tagName="span"
                      value={testimonial.name}
                      onBlur={(v) => updateTestimonial(index, 'name', v)}
                      isEditing={isEditing}
<<<<<<< HEAD
                      className="title-font font-medium text-[color:var(--color-text,#111827)]"
                      style={{ 
                        color: block.style?.color || 'var(--color-text, #111827)', 
=======
                      className="title-font font-medium text-gray-900"
                      style={{ 
                        color: block.style?.color, 
>>>>>>> khadija
                        fontFamily: block.style?.fontFamily,
                        ...content.nameStyle 
                      }}
                    />
                    <InlineEditableText
                      tagName="span"
                      value={testimonial.role}
                      onBlur={(v) => updateTestimonial(index, 'role', v)}
                      isEditing={isEditing}
<<<<<<< HEAD
                      className="text-[color:var(--color-muted,#6b7280)] text-sm uppercase"
                      style={{ 
                        color: block.style?.color || 'var(--color-text, #111827)', 
=======
                      className="text-gray-500 text-sm uppercase"
                      style={{ 
                        color: block.style?.color, 
>>>>>>> khadija
                        fontFamily: block.style?.fontFamily,
                        ...content.roleStyle 
                      }}
                    />
                  </span>
                </div>
<<<<<<< HEAD
              </EditableElement>
=======
              </div>
>>>>>>> khadija
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};
